...
<head>
    ...
    <title>Masuk - Eventix THEATRE</title>
    ...
    <div class="flex-grow flex items-center justify-center py-12 px-4">
        <div class="bg-white p-10 rounded-sm border border-gray-200 shadow-sm w-full max-w-md">
            <div class="text-center mb-8">
                <h2 class="text-3xl font-serif font-bold text-gray-900 mb-2">Masuk</h2>
                <p class="text-gray-500 text-sm">Masuk ke akun Anda untuk melihat tiket.</p>
            </div>
            
            @if(session('success'))
                <div class="bg-green-50 border border-green-200 text-green-700 p-3 rounded-sm mb-6 text-sm">{{ session('success') }}</div>
            @endif
            
            @if(session('error'))
                <div class="bg-red-50 border border-red-200 text-red-700 p-3 rounded-sm mb-6 text-sm">{{ session('error') }}</div>
            @endif
            ...
            <form action="{{ route('login') }}" method="POST" class="space-y-5">
                ...
                <div>
                    <div class="flex justify-between items-center mb-1">
                        <label class="block text-sm font-medium text-gray-700">Sandi</label>
                        <a href="{{ route('forgot-password') }}" class="text-xs text-theater-brown hover:underline">Lupa Sandi?</a>
                    </div>
                ...
                <button type="submit" class="w-full bg-theater-brown text-white font-bold tracking-wider py-3 px-4 rounded-sm hover:bg-[#7a4e35] transition-colors mt-2">MASUK</button>
            </form>
            <p class="text-center mt-8 text-sm text-gray-600">Belum punya akun? <a href="{{ route('register') }}" class="text-theater-brown font-semibold hover:underline">Daftar sekarang</a></p>
        </div>
    </div>
